...
if($check){
    if(move_uploaded_file($file_data['tmp_name'], $target_dir . $file_name)) { //will move the file
...
//loop through the supplier then add record.
        $suppliers = isset($_POST['suppliers']) ? $_POST['suppliers'] : []; //get suppliers
    ...
    $response = [
        'success' => true,
        'message' => "Product <strong>$product_name</strong> Successfully updated to the System."
    ];
} catch (\Exception $e){
    $response = [
        'success' => false,
        'message' => "<strong>$product_name</strong> Error updated."
    ];
}
